...
for p in [7,13,20]:
    ...
    for i in range(len(C_list)):
        cmd = '-v 5 -c '+ str(C_list[0])
        acc = train(y_train,x_train,cmd)
        if acc > best_acc:
            best_acc = acc
            bestC = C_list[i]
...
for p in [7,13,20]:
    for j in range(num): #测试num次
        ...
        x_test = np.array(x)[test_index].tolist()
        y_test = np.array(y)[test_index].tolist()
        cmd = '-c '+str(bestC_list[k])
        model = train(y_train,x_train,cmd) #在训练集上用最优参数训练模型
        t0 = datetime.now()
        _, p_acc, _ = predict(y_test, x_test, model) ##在测试集上测试模型，并计算时间和准确度
        t1 = datetime.now()
